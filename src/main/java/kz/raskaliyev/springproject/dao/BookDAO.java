...
public class BookDAO {
    ...
    public Optional<Person> getOwner(int bookId) {
        String query = "SELECT Person.* FROM Person JOIN Book ON Person.id = Book.person_id WHERE Book.id = ?";
        return jdbcTemplate.query(query, new Object[]{bookId}, new BeanPropertyRowMapper<>(Person.class))
                .stream().findFirst();
    }
